...
func send(c *net.UDPConn, dest *net.UDPAddr, response interface{}) {
	...
	// INTERIM HACK: for testing 'listen' with invalid message
	// listener, _ := net.ResolveUDPAddr("udp", "192.168.1.100:60001")
	// msg = []byte{
	// 	0x19, 0x20, 0x00, 0x00, 0xab, 0x91, 0x38, 0x19, 0xf9, 0x52, 0x00, 0x00, 0x02, 0x01, 0x02, 0x01,
	// 	0x01, 0x00, 0x00, 0x00, 0x20, 0x19, 0x12, 0x06, 0x21, 0x52, 0x35, 0x2c, 0x00, 0x00, 0x00, 0x00,
	// 	0x00, 0x00, 0x00, 0x00, 0x00, 0x21, 0x52, 0x35, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 	0x00, 0x02, 0x00, 0x19, 0x12, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	// }
	//
	// N, err = c.WriteTo(msg, listener)
	// if err != nil {
	// 	fmt.Printf("ERROR: %v\n", errors.New(fmt.Sprintf("Failed to write to UDP socket [%v]", err)))
	// } else {
	// 	if options.debug {
	// 		fmt.Printf(" ... sent %v bytes to %v\n%s\n", N, listener, dump(msg[0:N], " ...          "))
	// 	}
	// }
}
